...
export default function Page() {
  ...
  // States for Spedx pickup request
  const [showPickupForm, setShowPickupForm] = useState<boolean>(false);
  const [pickupService, setPickupService] = useState<string>("STANDART");
  ...
  async function handlePickupRequest() {
    if (!pickupDate) {
      setNotification({
        message: "Пожалуйста, выберите дату забора",
        type: "error",
      });
      return;
    }
    ...
    try {
      console.log("Sending pickup request with date:", pickupDate); // Debug log
      console.log("Sending pickup request with service:", pickupService); // Debug log
      ...
      if (pickupResponse.ErrorMessage === "Success") {
        setNotification({
          message: `Заявка на забор успешно создана! Номер заказа: ${pickupResponse.SpedxOrderNo}`,
          type: "success",
        });
        // Reset form
        setShowPickupForm(false);
        setPickupDate("");
        setPickupService("STANDART");
      } else {
        setNotification({
          message: `Ошибка: ${
            pickupResponse.ErrorMessageRu || pickupResponse.ErrorMessage
          }`,
          type: "error",
        });
      }
      ...
      setNotification({
        message: `Ошибка при создании заявки на забор: ${
          ...
          <ul className="list-disc pl-6">
            {trackingInfo.map((status: any, index: number) => (
              <li key={index} className="mb-4">
                <p>
                  <strong>Событие:</strong> {status.title}
                </p>
                <p>
                  <strong>Сообщение:</strong> {status.message}
                </p>
                <p>
                  <strong>Время:</strong> {status.eventTime}
                </p>
                <p>
                  <strong>Местоположение:</strong> {status.eventTown},{" "}
                  {status.country}
                </p>
              </li>
            ))}
                    ...
                    <ul className="list-disc pl-6">
                      {value.map((product: any) => (
                        <li key={product.id} className="mb-4">
                          ...
                          <p>
                            <strong>HS-код:</strong> {product.hsCode}
                          </p>
                          <p>
                            <strong>Удалено:</strong>{" "}
                            {product.isDeleted ? "Да" : "Нет"}
                          </p>
                        </li>
                      ))}
                    </ul>
  ...
  return (
    <div className="md:p-2 overflow-x-auto">
      ...
      <div className="mb-4 mt-4 flex flex-wrap items-center gap-4">
        ...
        >
          {showPickupForm ? "Скрыть" : "Создать заявку на забор"}
        </button>
      </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex-1">
                ...
                <select
                  ...
                  className="w-full border border-green-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                >
              ...
              <div className="flex-1">
                ...
                <DatePicker
                  ...
                  minDate={new Date()}
                  placeholder="Выберите дату забора"
                  className="w-full"
                />
              </div>
              <div className="flex items-end">
                <button
                  ...
                  disabled={pickupLoading || !pickupDate}
                ...
                >
                  {pickupLoading ? "Создание..." : "Создать заявку"}
                </button>
              </div>
            </div>
          </div>
              ...
              <tbody>
                {filteredShipments.map((shipment: any) => (
                  ...
                  >
                    ...
                    <td className="border border-gray-300 px-4 py-2">
                      {toDisplayDate(shipment.pickupDate!)}
                    </td>
                  </tr>
                ))}
              </tbody>
