...
import { UserWithRecipientsOverview } from '@/types/entities'

type Props = {
    users: UserWithRecipientsOverview[],
}

export default function UsersTable({users}: Readonly<Props>) {
    const router = useRouter()
    const statusText: { [key: string]: string } = useMemo(() => ({
        'NO_STATUS': 'Not created',
        'ACTIVE': 'Active',
        'INACTIVE': 'Inactive',
        'DELETED': 'Deleted',
        'PENDING': 'Pending',
    ...
    const columns = useMemo<ColumnDef<UserWithRecipientsOverview, any>[]>(
        () => [
            {
                accessorFn: row => capitalize(`${row.user.firstName.trim()} ${row.user.lastName.trim()}`),
                id: 'fullName',
                header: 'Full Name',
            ...
            {
                accessorFn: row => `${statusText[row.recipients[0]?.status ?? 'NO_STATUS']}`,
                id: 'status',
                header: 'Status',
                cell: info => {
                    return <div className={clsx('w-3 h-3 rounded-full', {
                        'bg-blue': info.getValue() === 'Active',
                        'bg-orange': info.getValue() === 'Inactive' || info.getValue() === 'Pending',
                        'bg-dark-gray': info.getValue() === 'Not created',
                    })}/>
                },
                footer: props => props.column.id,
            },
                            ...
                            {headerGroup.headers.map(header => {
                                return (
                                    <th key={header.id} colSpan={header.colSpan} className={clsx({
                                        'text-left': header.column.id === 'fullName',
